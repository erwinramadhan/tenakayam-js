...
const DetailTask = () => {
  ...
  const {detailTask} = useSelector(state => state);
  ...
  const onPressTask = (parentName, childName, isDone) => {
    setCtr(ctr + 1);
    dispatch(
      updateTaskList({
    ...
    if (isDone === false) {
      dispatch(addPoint(10));
    } else {
      dispatch(decreasePoint(10));
    }
  };
            ...
            {e.items.map((el, index) => (
              <View style={{paddingHorizontal: 0}} key={index}>
                <TouchableOpacity
                  ...
                  onPress={() => onPressTask(e.name, el.name, el.isDone)}>
